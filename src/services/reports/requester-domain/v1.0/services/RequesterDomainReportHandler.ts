...
export class RequesterDomainReportHandler implements IReportHandler {
    ...
    async download(params: DownloadReportParams): Promise<void> {
        ...
        if (type === ReportTypes.EXCEL) {
            const data = await RequesterDomainPreparation.getData(filters, language as Lang, false);
            const records = data.results.map((item, i) => ({
                index: i + 1,
                requesterName: item.requesterName,
                domain: item.domain,
                ticketCount: item.ticketCount,
            }));
            return GenericTabularReportDownload.downloadExcel({ report, records, type, language, user, res, filters, reportConfig: REQUESTER_DOMAIN_REPORT_CONFIG });
        }
